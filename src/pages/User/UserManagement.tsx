...
interface DistributorStatusDropdownProps {
  ...
  kycStatus: string; // New prop for KYC status
...
const DistributorStatusDropdown: React.FC<DistributorStatusDropdownProps> = ({
  ...
  // Only show Active and Rejected options when KYC is pending
  // If KYC is not pending, don't show dropdown at all
  const isKycPending = kycStatus?.toLowerCase() === "pending";

  const statusOptions = isKycPending
      ? [
          { value: "active", label: "Active", color: "text-[#806319]" },
          { value: "rejected", label: "Rejected", color: "text-[#b46055]" },
        ]
      : [
          { value: "pending", label: "Pending", color: "text-[#a06f13]" },
          { value: "active", label: "Active", color: "text-[#806319]" },
          { value: "rejected", label: "Rejected", color: "text-[#b46055]" },
        ];
  ...
  // If KYC is pending, show dropdown with only Active/Rejected
  // If KYC is NOT pending, show static badge (no dropdown)
  if (!isKycPending) {
      return (
          <span
...
> = ({
  ...
  // Get KYC status from business_profile
                      ...
                      <>
                          ...
                          {user.account_type ===
                              ...
                              <div className="mt-5 rounded-2xl border border-[#b8902e]/15 bg-gradient-to-br from-[#fffdfa] to-[#faf8f3] p-5">
                                  ...
                                  {/* Distributor Status in Detail Modal */}
...
const UserManagement: React.FC =
  () => {
      ...
      ] = useState<DistributorStatusFilter>("all");
          ...
          useMemo(() => {
              ...
              return users.filter(
                  (user) => {
                      ...
                      // Distributor Status Filter (only for distributors)
      ...
      const handleUpdateDistributorStatus = async (userId: number, newStatus: string) => {
          try {
              ...
              if (response.data.success) {
                  ...
                  toast.success(
                      response.data.message ||
                          `Distributor status updated to ${newStatus} successfully.`
                  );
              } else {
                  toast.error(
                      response.data.message ||
                          "Unable to update distributor status."
                  );
              }
          } catch (error: any) {
              console.error(
                  "Update distributor status error:",
                  error
              );
                                      ...
                                      return (
                                          ...
                                          >
                                              ...
                                              <div className="mt-3 flex flex-wrap items-center gap-2">
                                                  {/* User Status Dropdown for Mobile */}
                                                  <UserStatusDropdown
                                                      userId={user.id}
                                                      isActive={user.is_active}
                                                      onStatusChange={handleToggleUserStatus}
