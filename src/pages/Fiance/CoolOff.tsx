...
const normalizeRequestType = (type?: string | null) => {
  return String(type || "")
    .trim()
    .toLowerCase()
    .replace(/[-\s]+/g, "_");
...
const isCoolingOffRequest = (type?: string | null) => {
  return normalizeRequestType(type) === "cooling_off";
};
        ...
        <div className="space-y-5 p-5">
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-3">
            <p className="text-xs leading-5 text-[#8b3a34]">
              ⚠️ Rejecting will mark this Cooling-Off request as rejected. The
...
const MarkReceivedPopup: React.FC<
  MarkReceivedPopupProps
> = ({
  open,
  orderReference,
  customerName,
      ...
      <div className="w-full max-w-[470px] overflow-hidden rounded-2xl border border-[#163F20]/10 bg-white shadow-2xl">
        ...
        <div className="flex items-start justify-between gap-4 border-b border-[#D8E2D8] px-5 py-4">
          <div>
            ...
            <p className="mt-1 text-xs text-[#9AA29C]">
              Mark this Cooling-Off as received by the warehouse.
            </p>
          </div>
        ...
        <div className="space-y-4 p-5">
          ...
          <div className="rounded-xl border border-[#D8E2D8] bg-[#EAF3EA] p-3">
            <p className="text-xs leading-5 text-[#59645C]">
              ⚠️ By confirming, the Cooling-Off items will be marked as received
              by the warehouse. This action cannot be undone.
            </p>
          </div>
        </div>
...
const CoolOff: React.FC = () => {
  ...
  const fetchReturnRequests = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        // ONLY cooling_off requests
        const coolingOffRequests = list.filter(
          (item) => item.type === "cooling_off"
        );
  
        console.log(coolingOffRequests)
        setRequests(coolingOffRequests);
      } else {
        toast.error("Unable to fetch Cooling-Off requests.");
      }
    } catch (error: any) {
      console.error("Get Cooling-Off requests error:", error);
  
      toast.error(
        error?.response?.data?.message ||
          "Unable to fetch Cooling-Off requests."
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchReturnDetail = async (
    ...
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(
          "Unable to fetch Cooling-Off details."
        );
      }
    } catch (error: any) {
      console.error(
        "Cooling-Off detail error:",
        error
      );
      ...
      if (response.data.success) {
        toast.success(
          response.data.message ||
            "Cooling-Off approved successfully."
        );
        ...
        toast.error(
          response.data.message ||
            "Unable to approve Cooling-Off."
        );
      }
    } catch (error: any) {
      console.error(
        "Approve Cooling-Off error:",
        error
      );
  ...
  const handleReject = async (
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          response.data.message ||
            "Cooling-Off rejected successfully."
        );
        ...
        toast.error(
          response.data.message ||
            "Unable to reject Cooling-Off."
        );
      }
    } catch (error: any) {
      console.error(
        "Reject Cooling-Off error:",
        error
      );
  ...
  const handleOpenReceived = () => {
    ...
    if (
      ...
      toast.error(
        "Cooling-Off must be approved before marking as received."
      );
    ...
    async () => {
      ...
      try {
        ...
        if (response.data.success) {
          toast.success(
            response.data.message ||
              "Cooling-Off marked as received successfully."
          );
          ...
          toast.error(
            response.data.message ||
              "Unable to mark Cooling-Off as received."
          );
        }
      } catch (error: any) {
        console.error(
  ...
  const handleRefresh = async () => {
    ...
    toast.success(
      "Cooling-Off requests refreshed."
    );
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-5 flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div>
            ...
            <p className="mt-1 text-sm text-[#59645C]">
              Review Cooling-Off requests, approve refunds, and manage the
              Cooling-Off lifecycle.
            </p>
          </div>
        ...
        >
          <ReturnStatCard
            title="Total Cooling-Offs"
            value={stats.total}
            subtitle="All Cooling-Off requests"
            icon={
              <FiPackage size={21} />
            }
          ...
          <ReturnStatCard
            ...
            subtitle="Finished Cooling-Offs"
            icon={
              <FiCheck size={21} />
            }
