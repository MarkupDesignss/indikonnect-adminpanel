        ...
        <form onSubmit={handleSubmit}>
          <div className="space-y-4 p-5">
            ...
            <div className="rounded-xl border border-[#b8902e]/10 bg-[#fbfaf7] p-3">
              <div className="flex items-center gap-2">
                <FiLink size={14} className="text-[#b8902e]" />
...
const HeaderManagement: React.FC = () => {
  ...
  const fetchHeader = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        toast.error(
          response.data.message || "Unable to fetch header."
        );
      }
    } catch (error: any) {
  ...
  const homeMenu = useMemo(
    () =>
      menus.find(
        (menu) => menu.title.trim().toLowerCase() === "home"
      ),
    [menus]
  );
  ...
  const filteredMenus = useMemo(() => {
    ...
    return otherMenus
      .filter((menu) => {
        ...
        if (statusFilter === "active") return menu.status;
        if (statusFilter === "inactive") return !menu.status;
...
const handleSaveMenu = async (payload: MenuForm) => {
  try {
    ...
    if (response.data.success) {
      ...
      toast.error(
        response.data.message || "Unable to save menu."
      );
    }
  } catch (error: any) {
  ...
  const handleBrandingUpdate = async (payload: BrandingForm) => {
    ...
    try {
      ...
      const response = await headerApi.updateBranding(
        logoId,
        {
          logo: payload.logo,
      ...
      if (response.data.success) {
        ...
        toast.error(
          response.data.message || "Unable to update branding."
        );
      }
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        toast.success(
          response.data.message || "Menu deleted successfully."
        );
        ...
        toast.error(
          response.data.message || "Unable to delete menu."
        );
      }
    } catch (error: any) {
            ...
            <div className="flex items-center gap-3 rounded-xl border border-[#b8902e]/10 bg-white p-3">
              <div className="flex h-[70px] w-[130px] items-center justify-center overflow-hidden rounded-lg border border-[#b8902e]/10 bg-[#faf8f3] p-2">
                {logoUrl ? (
                  ...
                  <FiImage size={22} className="text-[#b9ae99]" />
                )}
              </div>
            ...
            <div className="flex items-center gap-3 rounded-xl border border-[#b8902e]/10 bg-white p-3">
              <div className="flex h-[70px] w-[70px] items-center justify-center overflow-hidden rounded-lg border border-[#b8902e]/10 bg-[#faf8f3] p-3">
                {faviconUrl ? (
                  ...
                  <FiGlobe size={22} className="text-[#b9ae99]" />
                )}
              </div>
            ...
            <div className="border-t border-[#b8902e]/10 bg-[#fffdfa] px-4 py-4">
              <div className="flex flex-col items-center justify-between gap-3 sm:flex-row">
                <p className="text-[10px] text-[#8b8171]">
                  Showing{" "}
                  <strong className="text-[#4a4436]">{startEntry}</strong> to{" "}
                  <strong className="text-[#4a4436]">{endEntry}</strong> of{" "}
